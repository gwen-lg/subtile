use super::{
    ...
    pds, PgsError, ReadExt, RleEncodedImage,
};
use std::{
...
mod tests {
    ...
    use crate::{image::ToImage, pgs::RleToImage};
    ...
    fn frame_image(frame: impl AsRef<Path>, img: impl AsRef<Path>) {
        let mut frame_file = File::open(frame).unwrap();
        let mut frame_buf: Vec<u8> = Vec::new();
        frame_file.read_to_end(&mut frame_buf).unwrap();
